...
jQuery( window.document ).ready(function($){
     ...
     * Message
     */
    $( '#branda-notice-permissions-settings .sui-notice-dismiss' ).on( 'click', function() {
...
function ub_media_bind( obj, event ) {
	...
	var type_error = container.next('.sui-notice-error');
	type_error.hide();
...
function ub_sui_notice( message, notice_class, dismiss ) {
    ...
    if ( "undefined" === typeof dismiss ) {
        dismiss = 'success' !== notice_class;
    }
...
jQuery( window ).bind( 'load', function( $ ) {
    if ( 'undefined' === typeof SUI.dialogs['branda-manage-all-modules'] ) {
        return;
    }

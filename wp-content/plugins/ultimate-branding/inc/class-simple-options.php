...
if ( ! class_exists( 'Simple_Options' ) ) {
	...
	class Simple_Options {
				...
				if ( preg_match( '/^(default|accordion)$/', $show_as ) ) {
					...
					if ( ! $section_inside ) {
						...
						if ( isset( $option['classes'] ) ) {
							...
							} else if ( is_array( $option['classes'] ) ) {
						...
						if ( preg_match( '/^(default|accordion)$/', $show_as ) ) {
							...
							if ( $show ) {
								if ( isset( $data['label'] ) || isset( $data['label_rich'] ) ) {
									...
									} else if ( isset( $data['label_rich'] ) && ! empty( $data['label_rich'] ) ) {
							...
							if ( isset( $data['options'] ) && is_array( $data['options'] ) ) {
								...
								foreach ( $data['options'] as $checkbox_value => $checkbox_label ) {
									...
									$content .= sprintf(
										...
										checked( 1, array_key_exists( $checkbox_value, $value ) ||
												isset( $data['always_selected'] ) && in_array( $checkbox_value, $data['always_selected'] ), false )
									);
									$content .= '<span></span>';
									$content .= sprintf(
										'<span class="sui-description">%s</span>',
								...
								if ( 1 === $columns ) {
									$content .= '</ul>';
								} else if ( 1 < $columns && 0 !== $counter % $columns ) {
									$content .= '</div>';
								}
							}
							break;
		...
		private function image( $section_key, $id, $images, $type, $module ) {
			...
			$element_id = sprintf( 'branda_option_media_%s_%s_%s',
				preg_replace( '/-/', '_', sanitize_title( $module ) ),
		...
		private function add_description( $data, $position = 'top' ) {
			$content = '';
			if ( ! isset( $data['description']['content'] ) ) {
				return $content;
			}

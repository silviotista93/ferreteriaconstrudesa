...
if ( ! class_exists( 'Branda_Admin' ) ) {
	...
	class Branda_Admin extends Branda_Base {
		...
		public function add_plugin_action_links( $actions, $plugin_file, $plugin_data, $context ) {
			...
			} else if ( ! $this->is_network || self::$is_show_admin_menu  ){
			...
			$links['docs'] = '<a href="https://premium.wpmudev.org/docs/wpmu-dev-plugins/branda/?utm_source=branda&utm_medium=plugin&utm_campaign=branda_pluginlist_docs" target="_blank">' . __( 'Docs', 'ub' ) . '</a>';
			if ( Branda_Helper::is_pro() ) {
				if ( ! Branda_Helper::is_member() ) {
					$links['renew'] = '<a href="https://premium.wpmudev.org/project/ultimate-branding/?utm_source=branda&utm_medium=plugin&utm_campaign=branda_pluginlist_renew" target="_blank" style="color: #8D00B1;">' . __( 'Renew Membership', 'ub' ) . '</a>';
				}
			} else {
				if ( is_network_admin() || ! is_multisite() ) {
					$url = 'https://premium.wpmudev.org/project/ultimate-branding/?utm_source=branda&utm_medium=plugin&utm_campaign=branda_pluginlist_upgrade';
		...
		public function plugin_row_meta( $plugin_meta, $plugin_file, $plugin_data, $status ) {
			...
			if ( $branda_plugin_file === $plugin_file ) {
				if ( Branda_Helper::is_pro() ) {
					$plugin_meta[2] =  '<a href="https://premium.wpmudev.org/project/ultimate-branding/" target="_blank">' . esc_html__( 'View Details', 'ub' ) . '</a>';
					$row_meta = array(
						'support' => '<a href="https://premium.wpmudev.org/hub/support/#wpmud-chat-pre-survey-modal" target="_blank">' . esc_html__( 'Premium Support', 'ub' ) . '</a>',
					);
				} else {
				...
				$row_meta['roadmap'] = '<a href="https://premium.wpmudev.org/roadmap/" target="_blank">' . esc_html__( 'Roadmap', 'ub' ) . '</a>';
		...
		public function add_admin_header_core() {
			...
			wp_enqueue_script( 'ub_admin', branda_url( $file ), array(
				'jquery',
		...
		private function handle_main_page_subsite() {
			...
			$args = array(
				...
				'groups' => branda_get_groups_list(),
				'modules' => $modules,
				'message' => apply_filters( 'branda_subsites_dashboard_message', array(
		...
		private function group_tabs_content( $modules, $current ) {
			...
			if ( ! $some_module_is_active ) {
				$template = 'admin/common/modules/no-module';

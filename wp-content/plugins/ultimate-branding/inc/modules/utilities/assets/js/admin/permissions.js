    ...
    jQuery('.branda-permissions-delete', container ).on( 'click', function( e ) {
        ...
        jQuery.post( ajaxurl, data, function( response ) {
            if ( response.success ) {
                ...
                window.ub_sui_notice( response.data.message, 'success', true );
            } else {
...
//	var languageInputTooShort = searchuser.data('language-input-tooshort');
	...
	searchuser.SUIselect2({
		...
		dropdownParent: $('.sui-dialog#branda-permissions-add-user'),
		ajax: {
			url: ajaxurl,
			type: "POST",
...
jQuery( window.document ).ready( function( $ ){
    ...
    $( '.branda-permissions-status input[type=checkbox].all' ).on( 'change', function() {
        ...
        if ( $(this).is(':checked') ) {
            $('input[type=checkbox]', $parent ).prop( 'checked', 'checked' );
        } else {

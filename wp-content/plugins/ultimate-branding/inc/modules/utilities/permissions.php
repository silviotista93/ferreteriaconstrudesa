...
if ( ! class_exists( 'Branda_Permissions' ) ) {
	...
	class Branda_Permissions extends Branda_Helper {
				...
				'permissions' => array(
					...
					'fields' => array(
						'user_roles' => array(
							...
							'type' => 'checkboxes',
							'columns' => 2,
							'disabled' => $branda_network ? ['super'] : ['administrator'],
							'always_selected' => $branda_network ? ['super'] : ['administrator'],
							'options' => $this->roles,
						),
						'users' => array(
							'type' => 'callback',
			...
			if ( $branda_network ) {
				$options['subsites'] = array(
					...
					'fields' => array(
						...
						'none' => array(
							'type' => 'notice',
							'value' => esc_html__( 'Subsite admins can\'t override any module settings and will always inherit your network settings.', 'ub' ),
							'master' => $this->get_name( 'status' ),
						...
						'all' => array(
							'type' => 'notice',
							'value' => esc_html__( 'Subsite admins can override any default module settings set in your network settings.', 'ub' ),
							'master' => $this->get_name( 'status' ),
		...
		public function get_users() {
			...
			$args = array(
				'button' => $this->button(
					array(
						'data' => array(
							'a11y-dialog-show' => $this->get_name( 'add-user' ),
							'nonce' => $nonce,
						),
						'icon' => 'plus',
						'text' => __( 'Add User', 'ub' ),
		...
		public function get_allowed_roles() {
			$roles = array_keys( $this->get_value('permissions', 'user_roles', [] ) );

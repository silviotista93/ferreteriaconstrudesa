	...
	class Branda_Admin_Menu extends Branda_Helper {
		...
		public function block_hidden_page() {
			...
			foreach ( $menu as $menu_item_index => $menu_item ) {
				...
				$is_menu_item_hidden = (boolean) branda_get_array_value( $menu_settings, array(
					$menu_item_id,
					'is_hidden',
				) );
				...
				if ( ! empty( $submenu[ $menu_slug ] ) ) {
					foreach ( $submenu[ $menu_slug ] as $submenu_item_index => $submenu_item ) {
						...
						$is_submenu_item_hidden = (boolean) branda_get_array_value( $menu_settings, array(
							$menu_item_id,
							'submenu',
							$submenu_item_id,
							'is_hidden',
		...
		private function remove_old_native_items( $menu ) {
			foreach ( $menu as $menu_item_id => $menu_item_settings ) {
				$is_native = (boolean) branda_get_array_value( $menu_item_settings, 'is_native' );
				$was_native = (boolean) branda_get_array_value( $menu_item_settings, 'was_native' );

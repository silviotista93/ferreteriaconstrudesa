				...
				'content'       => array(
					...
					'fields'      => array(
						...
						'privacy_policy_info'            => array(
							'type'         => 'notice',
							'value'        => __( 'The currently selected Privacy Policy page does not exist. Please create or select a new page.', 'ub' ),
		...
		public function add_cookie_notice() {
			...
			if ( function_exists( 'get_privacy_policy_url' ) ) {
				...
				if ( 'on' === $show ) {
					...
					if ( ! empty( $link ) ) {
						$content .= sprintf(
							'<span class="branda-cn-column"><a href="%s" class="button ub-cn-privacy-policy"%s>%s</a></span>',
							$link,
							$target,
